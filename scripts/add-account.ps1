...
function Add-Account
{
	...
	{
		connect-EWSService -Mail $Mailbox -ServiceUrl $ServiceUrl | out-null
	}
	
	elseif ($ServiceUrl -ne $null -and $Version -ne $null)
	{
		connect-EWSService -Mail $Mailbox -ServiceUrl $ServiceUrl -Version $Version | Out-Null
	}
	else
	{
		Write-Verbose -Message 'Connecting to EWS....'
		Connect-EWSSERVICE -mailbox $Mailbox -Version $Version | Out-Null
	}
}
